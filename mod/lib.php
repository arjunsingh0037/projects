...
define('HSUFORUM_CHOOSESUBSCRIBEBYADMIN', 4);
...
function hsuforum_user_can_post($forum, $discussion, $user=NULL, $cm=NULL, $course=NULL, $context=NULL) {
    ...
    // print_object($forum)|| die();
    // print_object($discussion)||die();
...
function hsuforum_print_latest_discussions($course, $forum, $maxdiscussions=-1, $sort='',
 ...
 // Sort out some defaults
    ...
    // print_object($maxdiscussions)||die();
    // exit();
...
function hsuforum_extend_settings_navigation(settings_navigation $settingsnav, navigation_node $forumnode) {
    ...
    if ($canmanage) 
    {
        ...
        // sangita 15 Nov 2017 Add subcription by admin functio

        $allowadminchoice = $mode->add(get_string('subscriptionbyadmin', 'hsuforum'), new moodle_url('/mod/hsuforum/subscribe.php', array('id'=>$forumobject->id, 'mode'=>HSUFORUM_CHOOSESUBSCRIBEBYADMIN, 'sesskey'=>sesskey())), navigation_node::TYPE_SETTING);

        // print_object($allowchoice);
        // print_object($forceforever);
        // print_object($forceinitially);
        // print_object($disallowchoice);
        // print_object($allowadminchoice);
        // exit();
        ...
        switch ($subscriptionmode) {
            ...
            case HSUFORUM_DISALLOWSUBSCRIBE : // 3
                ...
                //sangita 15 Nov 2017 check the admin subscription option

            case HSUFORUM_CHOOSESUBSCRIBEBYADMIN: // 4
                $allowadminchoice->action = null;
                $allowadminchoice->add_class('activesetting');
                $allowadminchoice->icon = new pix_icon('t/selected', '', 'mod_hsuforum');
                break;
        }
    ...
    else if ($activeenrolled)
    {
        ...
        switch ($subscriptionmode) {
            ...
            case HSUFORUM_DISALLOWSUBSCRIBE : // 3
                ...
                //sangita 15 Nov 2017 check admin subscription option condition

            case HSUFORUM_CHOOSESUBSCRIBEBYADMIN : // 4
                $notenode = $forumnode->add(get_string('subscriptionbyadmin', 'hsuforum'));
                break;    
       }
    }
