...
class mod_hsuforum_renderer extends plugin_renderer_base {
    ...
    public function discussion_template($d, $forumtype) {
        ...
        // Sangita 27 Nov 2017 : Add approve and approved button for discussion post by user
        ...
        $discussion = $DB->get_record('hsuforum_discussions', array('id' => $d->id), '*', MUST_EXIST);
        
         $statusvalue = $discussion->status;
         $disid = $discussion->id;
        ...
        if ($d->unread != '-') {
            ...
            $discussions = $DB->get_record('hsuforum_discussions', array('id' => $d->id), '*', MUST_EXIST);
            if($discussions->status == 1){
                $aprv='<input type="button" value="Approve" id="approveButton'.$disid.'" name="approve" onclick="statusupdate('.$disid.');"/>'; 
           }else{
                $aprv='<input type="button" value="Approved" id="approveButton'.$disid.'" name="approve" disbaled"/>'; 
           }
            ...
            echo '<script>
                            function statusupdate(id)
                            {
                                ...
                                ({
                                    ...
                                    {
                                        var abc = "<button type=button class=close data-dismiss=alert>&times;</button><p>Discussion Approved Successfully!!!</p>";
                                       document.getElementById("messageid"+id).innerHTML=abc;
                                       document.getElementById("messageid"+id).style.display="block";
                                       document.getElementById("approveButton"+id).value="Approved";
        ...
        if (!$d->fullthread) {
            if($statusvalue == 1){
                if(is_siteadmin()){
                    ...
                    $threadtitle = "<span class='disable-discussion'>$threadtitle</span>";
                }
...
HTML;
         ...
         {
            $displaymessage=get_string('postaddedbyuserdisplay','hsuforum');
...
HTML;
    ...
    else
    {
        ...
        <div class="alert alert-success alert-dismissable" id="messageid$disid" style="display: none;" >
...
HTML;
    ...
    protected function post_walker($cm, $discussion, $posts, $parent, $canreply, &$count, $depth = 0) {
       ...
       // print_object($discussion)||die(); course id.....
 ...
 //print_object($p)||die();
            ...
            // Sangita 26 Nov 2017 : Reply Message display

            $replystatusvalue='';
            $userreple='';
            $discussionid ='';
            $courseidingroup='';
            $replystatusvalue = $p->status;
          //  print_object($replystatusvalue)||die();
            $groupvalue='';
            $uservalue='';
            
            if($replystatusvalue==1)
            {
                $userreple=get_string('replymessagebyuser','hsuforum'); 
            }    
            else
            {
                $userreple= $p->message;
             ...
             $discussionvalue='';
             $discussionvalue=$p->discussionid;
            

              $discussionid = $DB->get_record('hsuforum_discussions', array('id' => $p->discussionid), '*', MUST_EXIST);
             
              $courseidingroup=$discussionid->course;
              
              $groupmember='';
              $totalrecords='';
              $totalgroup='';
              $totalvalue='';
              $groupvalue = $DB->get_records('groups', array('courseid' => $courseidingroup));
             
              $totalgroup = $DB->count_records('groups', array('courseid' => $courseidingroup));
              //print_object($totalgroup)||die();
          
             if($replystatusvalue== 2)
             { 
               
                     return <<<HTML
                     <div class="alert alert-success alert-dismissable" id="replymsgid$p->id" style="display: none" >
...
HTML;
         ...
         {    
            ...
            $groupmember='';
            $totalrecords='';
            //print_object($groupvalue)||die();
            foreach ($groupvalue as $record) 
            {
                
                $groupmember = $DB->get_records('groups_members', array('groupid' => $record->id));
                $totalrecords = $DB->count_records('groups_members', array('groupid' => $record->id));
        ...
        <fieldset>
            ...
            <input type="hidden" id="hsuforum-post-type" value="$postype"/>
                <label>
                    ...
                    <input type="text" placeholder="$t->subjectplaceholder" name="subject" class="form-control" $subjectrequired spellcheck="true" value="$subject" maxlength="255" /11>
                </label>
...
HTML;
    ...
    public function post_get_commands($post, $discussion, $cm, $canreply)
    {
        ...
        //Sangita 26 Nov 2017 : Add approve and approved button for approve reply by admin

        $parentvalue='';
        $parentvalue=$post->parent;
        ...
        if(is_siteadmin())
            {    
              ...
              {  
                ...
                if($post->status == 1){
                   $commands['approve'] ='&nbsp;<input type="button" value="Approve" id="replybutton'.$replyid.'" name="reply"  class = "hsuforum-reply-link btn btn-default"  onclick="replyupdate('.$replyid.');">';            
                }else{
                    
                    $commands['approve'] ='&nbsp;<input type="button" value="approved" id="replybutton'.$replyid.'" name="reply"  class = "hsuforum-reply-link btn btn-default" >';
                }    
                echo '    <script>
                                function replyupdate(id)
                                {
                                    ...
                                    ({
                                        ...
                                        success : function(data)
                                        {
                                           document.getElementById("replymsgid"+id).innerHTML="<button type=button class=close data-dismiss=alert>&times;</button>Reply Approved Successfully!!!";
                                       document.getElementById("replymsgid"+id).style.display="block";
                                       document.getElementById("replybutton"+id).value="Approved";
                                        }
                                    });
                                }   
                        </script>';
