...
//	Get list of unanswered questions, allow per-category if QA_ALLOW_UNINDEXED_QUERIES set in qa-config.php
	...
	switch ($by) {
		case 'selected':
			if ($countslugs) {
				$sometitle=qa_lang_html('main/unselected_qs');
				$nonetitle=qa_lang_html('main/no_una_questions');
		...
		case 'upvotes':
			if ($countslugs) {
				$sometitle=qa_lang_html('main/unupvoteda_qs');
				$nonetitle=qa_lang_html('main/no_una_questions');
		...
		default:
			...
			if ($countslugs) {
				$sometitle=qa_lang_html('main/unanswered_qs');
				$nonetitle=qa_lang_html('main/no_una_questions');
