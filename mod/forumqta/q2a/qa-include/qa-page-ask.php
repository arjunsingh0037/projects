...
//	Check whether this is a follow-on question and get some info we need from the database
	...
	//Obtaining the categoryid from the Q2A database
	/*$conn_ = mysql_connect(QA_FINAL_MYSQL_HOSTNAME, QA_FINAL_MYSQL_USERNAME, QA_FINAL_MYSQL_PASSWORD);
	mysql_select_db('moodle', $conn_);
	$sql_ = "SELECT categoryid FROM qa_categories WHERE tags ='".$category_tags."'";
	$result_ = mysql_query($sql_);
	$cat_= mysql_fetch_assoc($result_);*/
    global $DB;
    $category = $DB->get_record('forumqta_categories', array('tags'=>$category_tags));
	
	$followpostid=qa_get('follow');
	$in['categoryid']=qa_clicked('doask') ? $category->categoryid : qa_get('cat');
	...
	list($categories, $followanswer, $completetags)=qa_db_select_with_pending(
		...
		qa_db_popular_tags_selectspec(0, QA_DB_RETRIEVE_COMPLETE_TAGS, $category_tags)
	);
