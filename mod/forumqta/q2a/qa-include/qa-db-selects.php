...
*/
	{
        ...
        if ($groupid <= 0) { //all participants OR without seperated group
            return array(
                ...
                'source' => '^words JOIN (SELECT DISTINCT(qw.wordid) FROM ^words qw, ^tagwords qt, ^posts qp, ^categories qc WHERE tagcount>0 AND qc.tags = \''.$categorytags.'\' AND qw.wordid = qt.wordid AND qt.postid = qp.postid AND qc.categoryid = qp.categoryid ORDER BY tagcount DESC LIMIT #,#) y ON ^words.wordid=y.wordid',
            ...
            return array(
                'columns' => array('word', 'tagcount'),
                'source' => '^words JOIN (SELECT DISTINCT(qw.wordid) FROM ^words qw, ^tagwords qt, ^posts qp, ^categories qc WHERE (qp.groupid = '.$groupid.' OR qp.groupid = 0) AND tagcount>0 AND qc.tags = \''.$categorytags.'\' AND qw.wordid = qt.wordid AND qt.postid = qp.postid AND qc.categoryid = qp.categoryid ORDER BY tagcount DESC LIMIT #,#) y ON ^words.wordid=y.wordid',
                'arguments' => array($start, $count),
                'arraykey' => 'word',
                'arrayvalue' => 'tagcount',
                'sortdesc' => 'tagcount',
