...
*/
	{
		...
		$fixed_defaults=array(
			...
			'permit_edit_a' => QA_PERMIT_EDITORS,
			'permit_edit_c' => QA_PERMIT_EDITORS,
			'permit_edit_q' => QA_PERMIT_EDITORS,
			'permit_edit_silent' => QA_PERMIT_MODERATORS,
			'permit_flag' => QA_PERMIT_EXPERTS,
			'permit_hide_show' => QA_PERMIT_EDITORS,
			'permit_moderate' => QA_PERMIT_EDITORS,
			'permit_post_wall' => QA_PERMIT_USERS,
			'permit_select_a' => QA_PERMIT_EDITORS,
			'permit_view_q_page' => QA_PERMIT_USERS,
			'permit_view_voters_flaggers' => QA_PERMIT_ADMINS,
			'permit_vote_a' => QA_PERMIT_USERS,
			'permit_vote_down' => QA_PERMIT_EXPERTS,
			'permit_vote_q' => QA_PERMIT_USERS,
            'permit_post_q' => QA_PERMIT_EXPERTS,
            'permit_post_a' => QA_PERMIT_EXPERTS,
            'permit_post_c' => QA_PERMIT_EXPERTS,
			'points_a_selected' => 0,
		...
		if (isset($fixed_defaults[$name]))
			...
			switch ($name) {
				...
				/*case 'permit_post_q': // convert from deprecated option if available
					$value=qa_opt('ask_needs_login') ? QA_USER_LEVEL_EXPERT : QA_USER_LEVEL_EXPERT;
					break;
					
				case 'permit_post_a': // convert from deprecated option if available
					$value=qa_opt('answer_needs_login') ? QA_USER_LEVEL_EXPERT : QA_USER_LEVEL_EXPERT;
					break;
				
				case 'permit_post_c': // convert from deprecated option if available
					$value=qa_opt('comment_needs_login') ? QA_USER_LEVEL_EXPERT : QA_USER_LEVEL_EXPERT;
					break;
				*/
