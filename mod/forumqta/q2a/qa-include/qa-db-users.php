...
*/
    ...
    function qa_db_get_ranking($categoryid) {
        ...
        if ($groupid <= 0) {
            $nbPostsByUser =  qa_db_read_all_assoc(qa_db_query_sub(
                "SELECT COUNT(postid) AS counted,userid FROM ^posts WHERE categoryid=# GROUP BY userid ORDER BY counted DESC",
                $categoryid
            ));
        }
        else {
            $nbPostsByUser =  qa_db_read_all_assoc(qa_db_query_sub(
                "SELECT COUNT(postid) AS counted,userid FROM ^posts WHERE categoryid=# AND groupid=# GROUP BY userid ORDER BY counted DESC",
                $categoryid, $groupid
            ));
        }
        return $nbPostsByUser ;
    }

    function qa_db_get_ranking_paged($start, $count=null, $categoryid) {
        ...
        if ($groupid <= 0) {
            $nbPostsByUser =  qa_db_read_all_assoc(qa_db_query_sub(
                "SELECT COUNT(postid) AS counted,userid FROM ^posts WHERE categoryid=# GROUP BY userid ORDER BY counted DESC LIMIT #,#",
                $categoryid, $start, $count
            ));
        }
        else {
            $nbPostsByUser =  qa_db_read_all_assoc(qa_db_query_sub(
                "SELECT COUNT(postid) AS counted,userid FROM ^posts WHERE categoryid=# AND groupid=# GROUP BY userid ORDER BY counted DESC LIMIT #,#",
                $categoryid, $groupid, $start, $count
            ));
        }
        return $nbPostsByUser ;
    }

    function qa_db_get_users($categoryid) {
        $groupid = qa_get('groupid');
        if ($groupid <= 0) {
            $nbUsers =  qa_db_read_all_assoc(qa_db_query_sub(
                "SELECT DISTINCT(userid) FROM ^posts WHERE categoryid=#",
                $categoryid
            ));
        }
        else {
            $nbUsers =  qa_db_read_all_assoc(qa_db_query_sub(
                "SELECT DISTINCT(userid) FROM ^posts WHERE categoryid=# AND groupid=#",
                $categoryid, $groupid
            ));
        }
        return $nbUsers ;
